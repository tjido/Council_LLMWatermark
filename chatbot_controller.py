...
class Controller(LLMController):
    ...
    def select_responses(self, context: AgentContext) -> List[ScoredChatMessage]:
        ...
        system_prompt = "You are a professional whose job is to create a comprehensive report answering the user query based on data from various sources."

        controller_select_response_prompt = Template("""
        # Instructions
        - The provided context is a collection of information extracted from a PDF file answering the user query from the same source.
        - Your objective is to consolidate the data into a single report to answer the query comprehensively.
        - Analyze the data carefully and present it coherently in the final report.
        - Highlight any significant points or patterns found in the data.
